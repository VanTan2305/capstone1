...
//     if (type) {
//       var posts = await Post.aggregate(filter);
//     } else {
...
//     var decoded = jwt.decode(accessToken, process.env.ACCESS_TOKEN_SECRET, {
//       complete: true,
//     });
//     const owner = decoded.id;
//     const user = await User.findById(decoded.id);
//     if (!user)
//       return res.status(400).json({ msg: "User not available on system" });
//     const post = new Post({
//       owner,
//       title,
//       type,
...
const PostsController = {
  show: async (req, res) => {
    ...
    if (search && filter.length != 0)
      filter[0].$match.title = {
        $regex: search.replace(/  +/g, " ").trim().toLowerCase(),
        $options: "i",
      };
    else
      filter.push({
  ...
  addComment: async (req, res) => {
    ...
    try {
      ...
      if (blogDetail) {
        ...
        const blogDetailUpdated = await Post.update(
          { _id: req.params.id },
          {
            $set: {
              owner: owner,
